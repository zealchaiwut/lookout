...
Race checkpoints — traced from `tests/test_race_checkpoints_table_and_model__707.py` through 2 source file(s).

## Entry Points

- `tests/test_race_checkpoints_table_and_model__707.py` (tracing origin)
...
<!-- OPEN QUESTION: `pytest` imported in `test_race_checkpoints_table_and_model__707.py` but `pytest.py` not found in source — handler unresolved -->
<!-- OPEN QUESTION: `sqlalchemy` imported in `test_race_checkpoints_table_and_model__707.py` but `sqlalchemy.py` not found in source — handler unresolved -->
