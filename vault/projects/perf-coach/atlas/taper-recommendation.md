...
Taper recommendation — traced from `tests/test_taper_recommendation__608.py` through 5 source file(s).

## Entry Points

- `tests/test_taper_recommendation__608.py` (tracing origin)
...
<!-- OPEN QUESTION: `pytest` imported in `test_taper_recommendation__608.py` but `pytest.py` not found in source — handler unresolved -->
