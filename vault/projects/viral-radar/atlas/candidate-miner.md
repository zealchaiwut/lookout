---
feature: candidate-miner
files_read:
  - __init__.py
  - features.py
  - settings.py
  - candidate_miner.py
  - engagement_tier.py
traced: 2026-09-10
stale: false
---


## What

candidate-miner — traced from `tests/test_candidate_miner__22.py` through 5 source file(s).

## Entry Points

- `tests/test_candidate_miner__22.py` (tracing origin)

## Related Issues

_No related issues found in snapshot._

## Flowchart

```mermaid
flowchart LR
  __init___py[__init__.py]
  features_py[features.py]
  settings_py[settings.py]
  candidate_miner_py[candidate_miner.py]
  engagement_tier_py[engagement_tier.py]
  __init___py --> features_py
  features_py --> settings_py
  settings_py --> candidate_miner_py
  candidate_miner_py --> engagement_tier_py
```

## Key Files

- `__init__.py` — traced during import walk
- `features.py` — traced during import walk
- `settings.py` — traced during import walk
- `candidate_miner.py` — traced during import walk
- `engagement_tier.py` — traced during import walk
