...
Documented GET surfaces from the latest snapshot, grouped by path prefix. **Atlas / Handler** fill only when an atlas note cites that path (deterministic join — no live server calls). Currently **4/53** paths have an atlas join — the rest show `—` until `atlas_trace` / feature notes cover them.
